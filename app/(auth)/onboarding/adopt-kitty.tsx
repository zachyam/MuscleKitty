...
import { OnboardingService } from '@/app/service/onboarding/OnboardingService';
...
export default function AdoptKittyScreen() {
  ...
  const { isFirstLogin, user } = useUser();
  ...
  const handleAdopt = async () => {
    ...
    // Save the selected kitty using the service
    if (user?.id) {
      await OnboardingService.saveSelectedKitty(user.id, selectedKitty.id);
    }
    ...
    Animated.timing(fadeAnim, {
      ...
      router.push('/(auth)/onboarding/name-kitty');
    });
  };
