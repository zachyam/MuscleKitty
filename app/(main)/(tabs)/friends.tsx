...
import { FriendProfile, FriendshipStatus } from '@/app/repository/friends';
import * as KittyStats from '@/utils/kittyStats';
import FancyAlert from '@/components/FancyAlert';
import { KITTY_IMAGES, KITTY_ID_TO_BREED } from '@/app/(auth)/onboarding/name-kitty';
...
export default function FriendsScreen() {
  ...
  useEffect(() => {
    const loadUserAndFriends = async () => {
      ...
      try {
        ...
        if (!kittyHash) {
          kittyHash = await FriendService.getCurrentUserKittyHash(user.id);
        }
  ...
  const loadFriends = async () => {
    ...
    try {
      ...
      const profiles = await FriendService.getFriendProfiles(user.id);
  ...
  const handleAddFriend = async () => {
    ...
    try {
      ...
      const friendProfile = await FriendService.getFriendProfileByHash(newFriendId.trim());
      ...
      const success = await FriendService.addFriend(user.id, newFriendId.trim());
  ...
  const handleAcceptRequest = async (friend: Friend) => {
    ...
    try {
      const success = await FriendService.acceptFriendRequest(user.id, friend.userId);
  ...
  const handleRejectRequest = async (friend: Friend) => {
    ...
    try {
      const success = await FriendService.rejectFriendRequest(user.id, friend.userId);
  ...
  const handleRemoveFriend = async (friend: Friend) => {
    ...
    try {
      const success = await FriendService.removeFriend(user.id, friend.kittyHash);
