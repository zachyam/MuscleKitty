...
import { KITTY_GIFS } from '@/app/service/onboarding/OnboardingService';
...
function WorkoutPlansScreen() {
  ...
  const checkWorkoutInProgress = async () => {
    try {
      const workoutInProgress = await LocalStorageService.getWorkoutInProgress();
  ...
  const loadWorkouts = async () => {
    ...
    if (user?.id) {
      ...
      const savedWorkouts = await WorkoutService.getFormattedWorkouts(user.id);
