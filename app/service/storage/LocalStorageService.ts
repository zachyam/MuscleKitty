...
export class LocalStorageService {
  ...
  static async getWorkoutLogById(id: string): Promise<WorkoutLog | null> {
    try {
  ...
  static async getLatestWorkoutLog(workoutId: string, userId?: string): Promise<WorkoutLog | null> {
    try {
  ...
  static async saveWorkoutLog(log: WorkoutLog): Promise<void> {
    try {
      const logs = await this.getWorkoutLogs();
  ...
  static async getExerciseHistory(workoutId: string, exerciseId: string, userId?: string, limit = 10): Promise<{date: string, maxWeight: number}[]> {
    try {
      const logs = await this.getWorkoutLogs(userId);
  ...
  // Workout in Progress
  static async saveWorkoutInProgress(workoutLog: WorkoutLog): Promise<void> {
    try {
  ...
  static async getWorkoutInProgress(): Promise<WorkoutInProgress | null> {
    try {
      ...
      if (!workoutInProgress || !workoutInProgress.workoutLog || !workoutInProgress.startTime) {
        ...
        await this.clearWorkoutInProgress();
        return null;
      }
      ...
      await this.clearWorkoutInProgress(); // Clear on error to prevent persistent errors
      return null;
    }
  }

  static async clearWorkoutInProgress(): Promise<void> {
    try {
