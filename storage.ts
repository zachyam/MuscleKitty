...
// TEMPORARY: To generate fake workout data
// REMOVE THIS FUNCTION WHEN NO LONGER NEEDED
export const generateFakeWorkoutLogs = async (userId: string): Promise<void> => {
  try {
    // Clear existing logs to ensure we see our fake data
    await AsyncStorage.setItem(WORKOUT_LOGS_KEY, JSON.stringify([]));
    
    console.log('Cleared existing workout logs to generate fresh fake data.');
    ...
    // Set specific days so we know exactly what to expect
    // Generate workouts for every other day in the last 90 days
    const workoutDates = [];
    for (let i = 0; i < 10; i++) {
      const randomDaysAgo = Math.floor(Math.random() * 90); // Random day within the past 90 days
      const workoutDate = new Date();
      workoutDate.setDate(today.getDate() - randomDaysAgo);
      workoutDates.push(workoutDate);
    }
    
    console.log(`Generated ${workoutDates.length} workout dates`);
    
    // Create workout log for each date
    workoutDates.forEach((logDate, i) => {
      const log = {
        id: `fake-log-${i}`,
        workoutId: fakeWorkout.id,
        workoutName: fakeWorkout.name,
        exercises: [
          {
            exerciseId: "ex1",
            exerciseName: "Push-ups",
            sets: [
              { setNumber: 1, reps: 10, weight: i*2 },
              { setNumber: 2, reps: 10, weight: i*3 },
              { setNumber: 3, reps: 10, weight: i*4 }
            ]
          },
          {
            exerciseId: "ex2",
            exerciseName: "Squats",
            sets: [
              { setNumber: 1, reps: 15, weight: i*2 },
              { setNumber: 2, reps: 15, weight: i*3 },
              { setNumber: 3, reps: 15, weight: i*4 }
            ]
          }
        ],
    ...
    console.log('Fake workout logs generated successfully');
  } catch (error) {
    console.error('Error generating fake workout logs:', error);
  }
};
